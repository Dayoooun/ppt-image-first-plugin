...
이 단계는 `spec_lock.md` 확정 후, 본격적인 이미지 생성 직전에 실행한다. 목적: 각 슬라이드의 **요소 배치·계층·텍스트 정확 위치**를 HTML로 가볍게 잡아두고, 그것을 Codex `$imagegen` / GPT-Image 2.0 프롬프트의 spatial 청사진으로 사용한다. 이미지가 곧 최종 산출물이고, HTML은 청사진/검증 도구다.

중요: 각 슬라이드는 콘텐츠와 레퍼런스 이미지를 분리해서 순차 적용하지 않는다. 콘텐츠 잠금, 레퍼런스 이미지 경로, 스타일 규칙, 이미지 충실도 규칙, 금지 요소, 구도 지시를 **한 번의 통합 프롬프트**로 묶어 GPT-Image 2.0에 전달한다.
